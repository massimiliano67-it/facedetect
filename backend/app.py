...
@app.post("/api/delete-clusters")
def delete_clusters(req: DeleteClustersRequest):
    global ALL_FACES_DB, CLUSTER_NAMES
    ...
    # 2. Recopilar todos los IDs de caras de los clusters seleccionados
    for cid in req.cluster_ids:
        if cid in current_structure:
            for face in current_structure[cid]["faces"]:
                ids_to_delete.add(face["id"])
        
        # Eliminar nombre si existe
        if cid in CLUSTER_NAMES:
            del CLUSTER_NAMES[cid]

    # 3. Filtrar la base de datos global
    ALL_FACES_DB = [f for f in ALL_FACES_DB if f["id"] not in ids_to_delete]
